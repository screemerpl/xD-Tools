...
class RecordDialog(QDialog):
    ...
    def is_busy(self) -> bool:
        """Whether a recording is actually running -- what decides that
        the window's X hides instead of closes (see
        panels/hideable_dialog.py's close_hides_while_busy()).

        The recording itself, not the titling that follows it: titling
        happens in a nested modal dialog of its own, so this window's X
        is not reachable while it runs."""
