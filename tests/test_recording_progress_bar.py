...
def test_hidden_until_attached(qt_app):
    bar = RecordingProgressBar()
    assert not bar.isVisible()


def test_attach_shows_the_bar_before_anything_is_running(qt_app):
    """A dialog can be hidden before its operation ever starts, and this
    bar carries the only way back to a hidden window -- so it goes up as
    soon as a dialog is wired in, not on the first running_changed(True).
    Reported: opened a window, clicked Hide, and there was nothing left
    to click to get it back."""
    bar = RecordingProgressBar()
    bar.attach(track_progress=True)
    assert bar.isVisible()
    assert bar.overall_bar.value() == 0


def test_start_resets_the_values_without_re_showing(qt_app):
    bar = RecordingProgressBar()
    bar.attach(track_progress=True)
...
def test_track_progress_false_hides_the_track_row(qt_app):
    bar = RecordingProgressBar()
    bar.attach(track_progress=False)
