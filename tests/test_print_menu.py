...
def test_the_print_dialog_is_told_where_to_export(qt_app, monkeypatch, tmp_path):
    """Its Export PNG/PDF buttons should always open in XDProjects/printing,
    not wherever the app happened to be launched from -- and not beside the
    open project either, now that every export lands in one predictable
    place regardless of which project produced it."""
    monkeypatch.setattr(app_window_module, "PrintDialog", _FakeDialog)
    ...
    assert _FakeDialog.last_instance.start_dir == str(user_paths.printing_dir())
